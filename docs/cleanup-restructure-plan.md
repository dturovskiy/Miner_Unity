...
## Що Не Закрито По Cleanup

Cleanup уже хороший, але не фінальний.

Ще не закриті такі теми:

1. diagnostics усе ще шумні через часті `Heartbeat`, і це треба ще раз підтвердити новою runtime-сесією
2. hero-логи після нормалізації полів треба ще раз перевірити в новій сесії
3. `Stage 0` ще не можна закривати, поки не підтверджено чистий boot log і стабільне співпадіння cell-логіки героя з terrain-cell логікою
...
1. добити залишки `Stage 0`
2. після цього перейти в `Stage 1 - Ground Core`
